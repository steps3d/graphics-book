...
typedef void (APIENTRY * P_SOIL_GLCOMPRESSEDTEXIMAGE3DPROC) (GLenum target, GLint level, GLenum internalformat, GLsizei width, GLsizei height, GLsizei depth, GLint border, GLsizei imageSize, const GLvoid * data);
static P_SOIL_GLCOMPRESSEDTEXIMAGE3DPROC soilGlCompressedTexImage3D = NULL;
		...
		int loading_as_cubemap )
{
	...
	int mipmaps, cubemap, volume, uncompressed, block_size = 16;
	unsigned int flag;
    ...
    volume = header.dwFlags & DDSD_DEPTH;
	...
	if( uncompressed )
	{
		...
		DDS_main_size = depth * width * height * block_size;
	} else
	{
	...
	if( volume )
	{
		ogl_target_start = SOIL_TEXTURE_3D;
		ogl_target_end =   SOIL_TEXTURE_3D;
		opengl_texture_type = SOIL_TEXTURE_3D;
	} else
	if( cubemap )
	{
	...
	if( (header.sCaps.dwCaps1 & DDSCAPS_MIPMAP) && (header.dwMipMapCount > 1) )
	{
		...
		for( i = 1; i <= mipmaps; ++ i )
		{
			...
			DDS_full_size += d*w*h*block_size;
		}
	} else
	{
		mipmaps = 0;
	...
	for( cf_target = ogl_target_start; cf_target <= ogl_target_end; ++cf_target )
	{
		if( buffer_index + DDS_full_size <= buffer_length )
		{
			...
			if( uncompressed )
			{
				...
				if (volume)
					glTexImage3D(cf_target, 0, S3TC_type, 
			...
			} else
			{
				...
				glCompressedTexImage3D(
					cf_target, 0,
					S3TC_type, width, height, depth, 0,
			...
			for( i = 1; i <= mipmaps; ++i )
			{
				...
				{
					mip_size = d*((w+3)/4)*((h+3)/4)*block_size;
					if (volume)
					glCompressedTexImage3D(
						cf_target, i,
						S3TC_type, w, h, d, 0,
...
int checkExtension ( const char * ext )
{
	...
	for ( i = 0; i < num; i++ )
		if ( !strcmp ( (const char *)glGetStringi ( GL_EXTENSIONS, i ), ext ) )
			return 1;
...
int query_cubemap_capability( void )
{
#if 0		// in OGL 3 it is always supported
	/*	check for the capability	*/
	if( has_cubemap_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		/*	we haven't yet checked for the capability, do so	*/
		if( !glewIsSupported ( "GL_ARB_texture_cube_map" ) &&
			!glewIsSupported ( "GL_EXT_texture_cube_map" ) )
		{
			/*	not there, flag the failure	*/
	...
	if( has_DXT_capability == SOIL_CAPABILITY_UNKNOWN )
	{
		...
		} else
		{
...
#if	0		//__WIN32
			P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC ext_addr  = (P_SOIL_GLCOMPRESSEDTEXIMAGE2DPROC)getProcAddress ( "glCompressedTexImage2DARB" );
			P_SOIL_GLCOMPRESSEDTEXIMAGE3DPROC ext_addr2 = (P_SOIL_GLCOMPRESSEDTEXIMAGE3DPROC)getProcAddress ( "glCompressedTexImage3DARB" );
			PFNGLTEXIMAGE3DPROC 			  ext_addr3 = (PFNGLTEXIMAGE3DPROC)getProcAddress ( "glTexImage3D" );
			...
			} else
			{
				...
				soilGlCompressedTexImage3D = ext_addr2;
				glTexImage3D               = ext_addr3;
			}
#endif			
		}
	}
