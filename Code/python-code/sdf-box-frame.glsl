...
float boxRounded ( in vec3 pos, in vec3 size, float e )
{
    ...
    return min(min(
              length(max(vec3(pt.x, q.y, q.z),0.0))+min(max(pt.x,max(q.y,q.z)),0.0),
              length(max(vec3(q.x,  pt.y,q.z),0.0))+min(max(q.x,max(pt.y,q.z)),0.0)),
              length(max(vec3(q.x,  q.y, pt.z),0.0))+min(max(q.x,max(q.y,pt.z)),0.0));
}

float dE ( in vec3 p )
{
    return boxRounded ( p, vec3 ( 0.5, 0.3, 0.1 ), 0.02 );
}
