...
public:
	ShadowWindow () : GlutRotateWindow ( 100, 100, 900, 900, "Shadow Mapping" ) 
	{
		...
		if ( !program2.loadProgram ( "shadow-2.glsl" ) )
		...
		shadowMap->bind(0);
		glTexParameteri( shadowMap->getTarget (), GL_TEXTURE_COMPARE_MODE, GL_COMPARE_REF_TO_TEXTURE );
		glTexParameteri( shadowMap->getTarget (), GL_TEXTURE_COMPARE_FUNC, GL_LESS );
		shadowMap->unbind();
	}
