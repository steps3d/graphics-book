...
*/
	...
	Texture * createDepthTexture     ( GLenum format = GL_DEPTH_COMPONENT, GLenum internalFormat = GL_DEPTH_COMPONENT, 
	                                   GLenum clamp = GL_CLAMP_TO_BORDER, int filter = filterNearest )
	{
		return createColorTexture ( format, internalFormat, clamp, filter );
	}

	Texture * createColorTextureArray ( int numLayers, GLenum format = GL_RGBA, GLenum internalFormat = GL_RGBA8, 
	                                    GLenum clamp = GL_REPEAT, int filter = filterLinear );
										 
	Texture * createDepthTextureArray ( int numLayers, GLenum format = GL_DEPTH_COMPONENT, GLenum internalFormat = GL_DEPTH_COMPONENT, 
	                                    GLenum clamp = GL_CLAMP_TO_BORDER, int filter = filterNearest )
	{
		return createColorTextureArray ( numLayers, format, internalFormat, clamp, filter );
	}
