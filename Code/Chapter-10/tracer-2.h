...
struct SurfaceData
{
	...
	float		glossiness;	// for soft reflections in [0,1], 0 - no soft reflections,
	Medium		medium;
};
...
class Texture 		// class for application of textures on objects
{
public:
	Texture () = default;
	virtual ~Texture () = default;

	virtual void apply ( HitData& hit, SurfaceData& surface ) const = 0;
};
...
public:
	SphericLight ( const glm::vec3& p, const glm::vec3& c, float r ) : LightSource ( c ), pos ( p ), radius ( r ) {}
